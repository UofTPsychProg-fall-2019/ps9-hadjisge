...
ipip.l <- ipip.l %>% 
  separate (trait, into=c('trait', 'trait_items'), sep="_") #separate trait and item

# Calculate averages for each participant (coded as RID) and trait:
ipip.comp <- ipip.l %>% 
  group_by(RID, trait) %>%
  summarise(mean_trait_scores = mean(trait_scores)) #mean scores per trait per RID
...
ipip.comp <- ipip %>% 
  select(RID, age, gender, exer, BMI, logMedInc) %>% #merge these columns from ipip
  left_join(ipip.comp)
...
ipip.comp$exer <- factor(ipip.comp$exer, levels = c('veryRarelyNever','less1mo','less1wk','1or2wk','3or5wk','more5wk'), ordered = TRUE) #makes exer a factor and organizes the levels in increasing frequency
...
exer.avg <- ipip.comp %>% 
  group_by(trait, exer) %>%
  summarise(avg = mean(mean_trait_scores), sem = (sd(mean_trait_scores)/sqrt(length(RID))))
...
age.avg <- ipip.comp %>% 
  group_by(trait) %>%
  summarise(corrcoef = cor(age, mean_trait_scores))
